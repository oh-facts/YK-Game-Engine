...
{
    return yk_look_at(camera->pos, yk_math_vec3f_add(camera->pos, YK_WORLD_FORWARD), YK_WORLD_UP);
}

void yk_camera2d_move_up(YK_Camera2d *camera, f4 distance)
{
    camera->pos = yk_math_vec3f_add(camera->pos, yk_math_vec3f_mul_s(YK_WORLD_UP, distance));
}

void yk_camera2d_move_left(YK_Camera2d *camera, f4 distance)
{
    camera->pos = yk_math_vec3f_add(camera->pos, yk_math_vec3f_mul_s(YK_WORLD_LEFT, distance));
}

void yk_camera2d_move_right(YK_Camera2d *camera, f4 distance)
{
    camera->pos = yk_math_vec3f_add(camera->pos, yk_math_vec3f_mul_s(YK_WORLD_RIGHT, distance));
}

void yk_camera2d_move_down(YK_Camera2d *camera, f4 distance)
{
    camera->pos = yk_math_vec3f_add(camera->pos, yk_math_vec3f_mul_s(YK_WORLD_DOWN, distance));
}